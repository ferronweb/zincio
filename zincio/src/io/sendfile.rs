...
async fn sendfile_exact_completion<'a, 'b>(
    ...
    while (total_from_file < len && !file_eof) || total_to_socket < total_from_file {
        let splice_from_file_len = usize::try_from((len - total_from_file).min(usize::MAX as u64))
            .map_err(|_| {
                std::io::Error::new(
        ...
        let splice_to_socket_len = usize::try_from(
            (total_from_file - total_to_socket).min(usize::MAX as u64),
        )
        .map_err(|_| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
...
async fn sendfile_exact_poll<'a, 'b>(
    ...
    let mut total = 0;
    while total < len {
        let len_to_copy = usize::try_from((len - total).min(usize::MAX as u64)).map_err(|_| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "integer conversion out of range",
            )
